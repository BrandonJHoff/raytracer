...
void DielectricMaterial::shade(Color& result,
						  ...
						  int depth) const
{
	Point position = ray.point(record.t);
	...
	if((depth < context.scene->maxDepth) && (attenuation > context.scene->attenuation))
	{
		...
		if(costheta2 < 0)
		{
			...
			{
				temp_record.obj->mat->shade(reflect_color, context, reflect_ray, temp_record, attenuation, depth + 1);
			}
		...
		else
		{
			...
			if(context.scene->object->hit(temp_record, context, reflect_ray))
			{
				temp_record.obj->mat->shade(reflect_color, context, reflect_ray, temp_record, Fr.max()*attenuation, depth + 1);
			}
			...
			Vector transp_direction = (ray.dir/indexTemp) + N*((costheta/indexTemp) - costheta2);
			...
			{
				temp_record.obj->mat->shade(transp_color, context, transp_ray, temp_record, Ft.max()*attenuation, depth + 1);
			}
